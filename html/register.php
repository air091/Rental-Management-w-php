...
<body>
  <div class="h-screen flex items-center justify-center">
    <div class="border border-stone-400 p-4 rounded-xl w-full max-w-72">
      <h1 class="text-2xl text-2xl text-center mb-2 font-medium">Register your Account</h1>
      <form method="POST" class="grid gap-y-2">

        <!-- REGISTER STATUS -->
        <?php if ($error): ?>
          <p class="text-red-700 text-center"><?= htmlspecialchars($error) ?></p>
        <?php endif ?>
        <!-- REGISTER STATUS -->
        ...
        <div>
          ...
          <input id="password" type="password" name="password" class="block border border-stone-400 px-2 py-1 w-full rounded-sm">
        </div>
